...
fun Application.eventServiceRoutes(eventServiceServices: EventServiceServices) {
    ...
    routing {
        ...
        delete("/event/{id}") {
            try {
                val id = call.pathParameters["id"]?.toLong() ?: return@delete call.respond(HttpStatusCode.BadRequest)
        ...
        get("/ratings/{id}"){
            try {
                val serviceId = call.parameters["id"]?.toLong() ?: return@get call.respond(HttpStatusCode.BadRequest)
        ...
        get("/servicesByProvider/{id}"){
            try {
                val serviceProviderId = call.parameters["id"]?.toLong() ?: return@get call.respond(HttpStatusCode.BadRequest)
